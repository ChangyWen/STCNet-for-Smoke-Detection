...
class STCNet(nn.Module):
    ...
    def forward(self, rgb, residual, target=None, is_testing=True):
        ...
        s2 = self.spatial_path.layer2(STCNet.op_fuse(s1, t1))
        t2 = self.temporal_path.layer2(STCNet.op_fuse(s1, t1))

        s3 = self.spatial_path.layer3(STCNet.op_fuse(s2, t2))
        t3 = self.temporal_path.layer3(STCNet.op_fuse(s2, t2))

        s4 = self.spatial_path.layer4(STCNet.op_fuse(s3, t3))
        t4 = self.temporal_path.layer4(STCNet.op_fuse(s3, t3))

        x0 = self.layer_cls(STCNet.op_fuse(s4, t4))
        x0 = x0.unflatten(dim=0, sizes=torch.Size([b, t]))
        x0 = STCNet.op_mean(x0)
